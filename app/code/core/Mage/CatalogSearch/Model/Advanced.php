...
class Mage_CatalogSearch_Model_Advanced extends Mage_Core_Model_Abstract
{
    ...
    public function addFilters($values)
    {
        ...
        if ($allConditions
            || (isset($values['category']) && is_numeric($values['category']))
            || (isset($_GET['created_at']) && (!empty($_GET['created_at']['from']) || !empty($_GET['created_at']['to'])))
            ){
    ...
    public function getSearchCriterias()
    {
        ...
        if(isset($_GET['category']) && is_numeric($_GET['category'])) {
            ...
            $search[] = array('name' => Mage::helper('catalog')->__('Category'),'value'=>$category->getName());
        }

        if(isset($_GET['created_at']) && (!empty($_GET['created_at']['from']) || !empty($_GET['created_at']['to']))) {
            $value = '';
            if(!empty($_GET['created_at']['from'])) {
                $value .= Mage::helper('catalog')->__('From: %s', $_GET['created_at']['from']);
                $value .= ' ';
            }

            if(!empty($_GET['created_at']['to'])) {
                $value .= Mage::helper('catalog')->__('To: %s', $_GET['created_at']['to']);
            }

            $search[] = array('name'=>Mage::helper('catalog')->__('Date'),'value'=>$value);
        }
    ...
    public function getProductCollection(){
        if (is_null($this->_productCollection)) {
            ...
            if(isset($_GET['created_at']) && (!empty($_GET['created_at']['from']) || !empty($_GET['created_at']['to']))) {
                if(!empty($_GET['created_at']['from'])) {
                    $date = new Zend_Date($_GET['created_at']['from'], 'MM/dd/yy');
                    $this->_productCollection->addAttributeToFilter('created_at', array('gteq' => $date->toString('YY-MM-dd 00:00:00')));
                }

                if(!empty($_GET['created_at']['to'])) {
                    $date = new Zend_Date($_GET['created_at']['to'], 'MM/dd/yy');
                    $this->_productCollection->addAttributeToFilter('created_at', array('lteq' => $date->toString('YY-MM-dd 23:59:59')));
                }
            }
        }
