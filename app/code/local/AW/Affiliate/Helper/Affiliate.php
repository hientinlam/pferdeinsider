...
class AW_Affiliate_Helper_Affiliate extends Mage_Core_Helper_Abstract
{
  ...
  public function createAffiliateLink($paramz)
    {
         /*if (!array_key_exists(self::CAMPIGN_REQUEST_KEY, $params) || !array_key_exists(self::AFFILIATE_REQUEST_KEY, $params)) {
            return null;
        }*/
        $cmid = Mage::helper('core')->encrypt($paramz[self::CAMPAIGN_REQUEST_KEY]);
        $afid = Mage::helper('core')->encrypt($paramz[self::AFFILIATE_REQUEST_KEY]);
        $ats = Mage::helper('core')->encrypt($paramz[self::AFFILIATE_TRAFFIC_SOURCE]);
